...
def load_scaler(filename="scaler.pkl"):
    ...
    return None
from joblib import dump, load  # ✅ Import load from joblib
import matplotlib.pyplot as plt
import math
import numpy as np
import pandas as pd
...
def LSTM_ALGO(df, quote):
    ...
    sc = load_scaler("scaler.pkl")
    if sc is None:
        sc = MinMaxScaler(feature_range=(0, 1))
        training_set_scaled = sc.fit_transform(training_set)
        save_scaler(sc, "scaler.pkl")
    else:
    ...
    regressor = load_lstm_model("lstm_model.h5")
    if regressor is None:
        ...
        save_lstm_model(regressor, "lstm_model.h5")
    ...
    plt.plot(dataset_test["Date"], real_stock_price, label="Actual Price", color='blue')
    plt.plot(dataset_test["Date"], predicted_stock_price, label="Predicted Price", color='red')
    plt.xlabel("Date")
    plt.ylabel("Stock Price")
    plt.xticks(rotation=45)
    plt.legend(loc="best")
    plt.title("Actual vs Predicted Stock Price")
    image_path = f"static/LSTM_{quote}.png"
...
def save_arima_model(model, filename="arima_model.pkl"):
    joblib.dump(model, filename)

def load_arima_model(filename="arima_model.pkl"):
    if os.path.exists(filename):
...
def ARIMA_ALGO(df, quote):
    try:
        ...
        def arima_model(train, test, days):
            ...
            loaded_model = load_arima_model(model_filename)

            if loaded_model:
                model_fit = loaded_model
            else:
                model = ARIMA(history, order=(6,1,0))
                model_fit = model.fit()
                save_arima_model(model_fit, model_filename)
            ...
            #     history.append(test[t])
            for t in range(len(test)):
                model = ARIMA(history, order=(6,1,0))  # Refit the model
                model_fit = model.fit()
        ...
        for company in uniqueVals[:1]:  
            data = df[df["Code"] == company].copy()
            data['Price'] = data['Close']
            Quantity_date = data[['Price', 'Date']].set_index("Date").fillna(method='ffill')
            ...
            image_path = f"static/ARIMA_{quote}.png"
            plt.savefig(image_path)
            plt.close()

            return arima_pred, error_arima, future_forecast  

    except Exception as e:
        print(f"❌ Error in ARIMA_ALGO: {e}")
        return None, None, None  
    
def save_lr_model(model, filename="linear_regression_model.pkl"):
    dump(model, filename)

def load_lr_model(filename="linear_regression_model.pkl"):
    if os.path.exists(filename):
        return load(filename)
    return None
def LIN_REG_ALGO(df, quote):
    ...
    clf = load_lr_model("linear_regression_model.pkl")
    if clf is None:
        clf = LinearRegression(n_jobs=-1)
        clf.fit(X_train, y_train)
        save_lr_model(clf, "linear_regression_model.pkl")
    ...
    plt2.legend(loc=4)
    image_path = f"static/LR_{quote}.png"
    plt.savefig(image_path)
    plt2.close()

    error_lr = math.sqrt(mean_squared_error(y_test, y_test_pred))

    forecast_set = clf.predict(X_to_be_forecasted) * 1.04
...
def get_stock_prediction(quote):
    ...
    #     print(f"❌ Error in ARIMA_ALGO: {e}")
    try:
        arima_pred, error_arima, arima_forecast = ARIMA_ALGO(df, quote)
    except Exception as e:
     arima_pred, error_arima, arima_forecast = None, None, []
     print(f"❌ Error in ARIMA_ALGO: {e}")

    try:
        predicted_price, error_lstm, future_prices = LSTM_ALGO(df, quote)
    except Exception as e:
